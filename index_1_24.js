...
//     if ([key]) {
//         return dino[key]; // bracket will extract the value of the key
//     } else {
//         return dino.dinosaurId;
//     }
//   }

// //if we key into an object if a key doesn't exist (invalid, no key), it will return undefined 

//   console.log(getValueByKeyFromDino(dinos, key))
  ...
  //pseudocode
  //return a boolean
  // if mya[0] === one then dino is dead
  // if mya[1] === dino is alive

  function isDinoAlive(dino, mya) {
    if (dino.mya.length === 1) {
        if (dino.mya[0] === dino.mya || dino.mya[0] -1 === mya) {
            return true;
        }
    } else {
...
console.log(function isDinoAlive(dino, mya))
  ...
  // If the dinosaur only has a single value for `mya`, they're considered alive if the `mya` value is equal to the given value or one less. For example, if a dinosaur has a `mya` value of `[29]`, the dinosaur's information will be returned if `29` is entered or `28` is entered.
        //mya === 80.5 || mya === 79.5 ---> living dino
        //mya [154, 133] mya === (154 - 133) --- dead dino
  // ------------------------------------------------------
  ...
  //for in loop syntax
//  for (let key of dinosaurId)

function transformDinosaurArray(dinosaurs) {
    const transformedArray = [];
    
    for (let dino of dinosaurs) {
        let value = dino[key] || dino.dinosaurID;
        transformDinosaursArray.push(value);
...
// return dinosaurs.map([dino)=> dubi[key] || dinosaurId];

/*
//Use a `for in` loop, which will give you keys to iterate. In the control panel of this `for` loop, the value key can be anything.  You can name it `asdf` or anything you like. `in` is a reserved keyword that defines what kind of for loop this is, and finally, the object you are iterating over is teh final piece of the control panel.
for (let dino in dinosaurs) {
    console.log(dino);
  //Write a loop that iterates over the object, and console logs 
  /Object.keys method &  for loop
//remember below?  
const programmerKeys = Object.keys(programmer);
console.log(programmerKeys);// becomes an array with the Object.keys method

for(let i = 0; i < programmerKeys.length; i++) {
  console.log(programmerKeys[i], programmer[programmerkeys[i]]);
}
